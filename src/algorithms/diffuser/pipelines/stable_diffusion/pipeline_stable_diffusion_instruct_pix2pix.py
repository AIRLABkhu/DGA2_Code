...
class StableDiffusionInstructPix2PixPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        latent_mask:PipelineImageInput=None,
        ...
        **kwargs
        ...
        #2.5 Inject noise in embedding
        noise_for_embedding= torch.randn_like(prompt_embeds) * torch.std(prompt_embeds) * noise_ratio
        prompt_embeds = prompt_embeds + noise_for_embedding
        prompt_embeds=prompt_embeds
        # 3. Preprocess image
        ...
        #Image Latent: [3,4,32,32]

        #Prepare the Mask
        masking_image_latents=image_latents.clone()[:1,:,:,:]
        latent_mask=torch.stack(latent_mask).to(device)
        ...
        for i, t in enumerate(timesteps):
            ...
            # three gaussian latents
            latent_model_input = torch.cat([latents] * 3) if self.do_classifier_free_guidance else latents

            #latent_model_input : [3,4,32,32]
            # concat latents, image_latents in the channel dimension
            scaled_latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
            scaled_latent_model_input = torch.cat([scaled_latent_model_input, image_latents], dim=1)
            # scaled_latent_model_input : [3,8,32,32]
            ...
            original_latent=self.scheduler.add_noise(masking_image_latents,torch.randn_like(masking_image_latents).to(device)
                                                        ,t.unsqueeze(0))

            latents=latent_mask*latents+(1-latent_mask)*original_latent
    ...
    """
    def _read_mask(self,mask_path:str,dest_size=(32,32)):
